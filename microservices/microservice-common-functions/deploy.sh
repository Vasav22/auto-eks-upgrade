...
# make sure to have aws cli setup on you local -- aws configure --profile opsera
...
$(aws secretsmanager get-secret-value --secret-id=$ENV:OPSERA_ENV --profile opsera | jq-win64 '.SecretString | fromjson' | jq-win64 -r 'keys[] as $k | \"$ENV:\($k)=\\\"\(.[$k])\\\"\"') >> env_out.ps1
